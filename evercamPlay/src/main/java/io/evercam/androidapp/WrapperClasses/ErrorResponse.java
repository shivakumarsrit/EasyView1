...
class ErrorResponse extends EvercamObject {
    ...
    ErrorResponse(String jsonObjectString) throws EvercamException {
        try{
    ...
    protected ArrayList<String> getContexts() throws EvercamException {
        ...
        JSONArray contextJsonArray = null;
        try {
            contextJsonArray = jsonObject.getJSONArray("context");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        if (contextJsonArray.length() != 0) {
            for (int index = 0; index < contextJsonArray.length(); index++) {
                String context = null;
                try {
                    context = contextJsonArray.getString(index);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
